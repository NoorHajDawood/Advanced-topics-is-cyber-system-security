...
{
    //printf("Start of http_read_line\n");
    size_t i = 0;
    ...
    for (;;)
    {
        ...
        {
            buf[i] = '\0';
            //printf("http_read_like newline in buf [return]\n");
            return 0;
        }
        ...
        {
            buf[i] = '\0';
            //printf("http_read_like end of buf [return]\n");
            return 0;
        }
...
const char *http_request_line(int fd, char *reqpath, char *env, size_t *env_len) // reqpath 4096 | env 8192
{
    ...
    if (http_read_line(fd, buf, sizeof(buf)) < 0) {
        //printf("http_request_line Socket IO error\n");
    ...
    sp1 = strchr(buf, ' ');
    if (!sp1) {
        //printf("http_request_line cannot parse HTTP request 1\n");
        return "Cannot parse HTTP request (1)";
    }
    *sp1 = '\0';
    sp1++;
    if (*sp1 != '/') {
        //printf("http_request_line Bad request path\n");
        return "Bad request path";
    }

    sp2 = strchr(sp1, ' ');
    if (!sp2) {
        //printf("http_request_line cannot parse HTTP request 2\n");
    ...
    *sp2 = '\0';
    ...
    if (strcmp(buf, "GET") && strcmp(buf, "POST")) {
        //printf("http_request_line unsupported request\n");
    ...
    url_decode(reqpath, sp1); // potential buffer overflow
    ...
    *envp = 0;
    *env_len = envp - env + 1;
    //printf("END http_request_line\n");
    return NULL;
}

const char *http_request_headers(int fd)
{
    //printf("START http_request_headers\n");
    static char buf[8192];      /* static variables are not on the stack */
    int i;
    char value[512];
    char envvar[512];
    ...
    for (;;)
    {
        ...
        *colon = '\0';
        ...
        url_decode(value, sp); // potential buffer overflow
        ...
        if (strcmp(buf, "CONTENT_TYPE") != 0 &&
            ...
            sprintf(envvar, "HTTP_%s", buf); // potential buffer overflow
            setenv(envvar, value, 1);
        } else {
            setenv(buf, value, 1);
        }
    }
    //printf("END http_request_headers\n");
    return 0;
}
...
void http_serve(int fd, const char *name)
{
    ...
    split_path(pn);
    if (!stat(pn, &st)) // stat returns information about a file information
    {
        /* executable bits -- run as CGI script */
        if (valid_cgi_script(&st)) {
            // printf("handler = http_serve_executable\n");
            handler = http_serve_executable;
        } else if (S_ISDIR(st.st_mode)) {
            // printf("handler = http_serve_directory\n");
            handler = http_serve_directory;
        } else {
            // printf("handler = http_serve_file\n");
...
void http_serve_directory(int fd, const char *pn) {
    //printf("Start of http_serve_directory\n");
    ...
    http_serve(fd, name);
    //printf("End of http_serve_directory\n");
}

void http_serve_executable(int fd, const char *pn)
{
    //printf("Start of http_serve_executable\n");
    ...
    //printf("End of http_serve_executable\n");
}
